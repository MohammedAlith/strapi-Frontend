...
export default async function AboutSection() {
  ...
  try {
    const res = await fetch(`${strapiUrl}/api/home-page?populate[AboutSection][populate]=*`,{ cache: 'force-cache' });
  ...
  return (
    <div className="grid grid-cols-2 gap-5">
      ...
      <div className="bg-white p-6 grid gap-4">
        ...
        />
        <h1 className="text-5xl font-bold">{data.AboutSection.title}</h1>
        <p className="text-xl text-gray-500">{data.AboutSection.intro}</p>
        <p className="text-md text-gray-500">{data.AboutSection.desc}</p>
...
></div>
 ...
 <div>
    <img
      ...
      alt="about-img1"
      className="rounded-lg absolute  top-40 h-74 w-2/4 z-50" 
    />
</div>
...
<div>
    <img
      ...
      alt="about-img2"
      className="rounded-lg h-3/4 absolute right-0"
    />
    </div>
